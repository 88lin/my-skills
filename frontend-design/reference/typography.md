...
**Avoid invisible defaults**: Inter, Roboto, Open Sans, Lato, Montserrat, Geist, Fraunces, Mona Sans, Plus Jakarta Sans, Space Grotesk, Recoleta, and Instrument Sans are now common enough to make many generated interfaces feel generic. They're fine for documentation or tools where personality is not the goal, but if you want distinctive design, look elsewhere.

- A technical/utilitarian brief does not need a serif "for warmth." Most tech tools should look like tech tools.
- An editorial/premium brief does not need the same expressive serif everyone is using right now. Premium can be Swiss-modern, neo-grotesque, monospace, or a quiet humanist sans.
...
**`swap` vs `optional`**: `swap` shows fallback text immediately and swaps when the web font arrives. `optional` uses the fallback if the web font misses a small load budget and avoids the shift entirely. Pick `optional` when zero layout shift matters more than seeing the branded font on slow networks.

**Preload the critical weight only**: usually the regular-weight body font used above the fold. Preloading every weight costs more bandwidth than it saves.
...
**Bound your clamp()**: keep `max-size` less than or equal to about 2.5 times `min-size`. Wider ratios break browser zoom and reflow behavior and make large viewports feel like the page is shouting.
